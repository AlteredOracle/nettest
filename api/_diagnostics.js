// Shared diagnostics engine — used by /api/diagnose and /api/tickets.
// Files prefixed with "_" are not treated as routes by Vercel; they're bundled
// into any function that requires them.
//
// Runs REAL checks: DNS resolution, TCP reachability, HTTPS response, and TLS
// certificate validity. ICMP ping / traceroute are intentionally absent —
// Vercel's runtime can't open raw sockets, so a TCP connect is the reachability
// test instead.
...
// Returns { target, resolvedIp, checks, diagnosis, durationMs } or { error }.
async function runDiagnostics(raw) {
  const parsed = parseTarget(raw);
  if (!parsed) return { error: 'Enter a valid hostname, IP, or URL (e.g. example.com or 1.1.1.1).' };
  ...
  checks.push({
    ...
    detail: dnsRes.ok ? `${dnsRes.addresses.join(', ')} · ${dnsRes.ms} ms` : `failed · ${dnsRes.err}`
  });
  ...
  checks.push({
    name: 'HTTPS response',
    status: httpsStatus,
    detail: httpsRes.status ? `HTTP ${httpsRes.status} · ${httpsRes.ms} ms` : `no response · ${httpsRes.err}`
  });

  let tlsStatus, tlsDetail;
  if (!tlsRes.ok && tlsRes.err) {
    tlsStatus = 'fault'; tlsDetail = `failed · ${tlsRes.err}`;
  } else if (tlsRes.daysLeft <= 0) {
    tlsStatus = 'fault'; tlsDetail = `EXPIRED ${Math.abs(tlsRes.daysLeft)}d ago · ${tlsRes.issuer}`;
  } else if (tlsRes.daysLeft <= 14) {
    tlsStatus = 'amber'; tlsDetail = `expires in ${tlsRes.daysLeft}d · ${tlsRes.issuer}`;
  } else if (!tlsRes.authorized) {
    tlsStatus = 'amber'; tlsDetail = `untrusted · ${tlsRes.authError || 'cert not trusted'}`;
  } else {
    tlsStatus = 'ok'; tlsDetail = `valid · expires in ${tlsRes.daysLeft}d · ${tlsRes.issuer}`;
  }
  ...
  return { target: host, resolvedIp, checks, diagnosis, durationMs: Date.now() - started };
}

module.exports = { runDiagnostics, parseTarget };
